...
def find_dataset():
    """
    Locate IndianData_final.csv - Modified for Jupyter Notebook
    """
    # Check current directory first
    if os.path.exists("IndianData_final.csv"):
        return "IndianData_final.csv"
    
    # Check common alternative locations
    possible_paths = [
        "Backend/IndianData_final.csv",
        "../IndianData_final.csv",
        "data/IndianData_final.csv"
    ]
    ...
    raise FileNotFoundError(
        "IndianData_final.csv not found. "
...
def load_data(filepath):
    ...
    # ==========================================
    # This is a categorical column already in dataset
    # Using this instead of occupation_status (which has low variation)
...
def create_time_windows(df):
    ...
    for k, v in windows.items():
        print(
            f"  {k}: {len(v):,} rows "
            f"({v.issue_date.min().date()} → {v.issue_date.max().date()})"
        )
...
# =========================================================
# 3. FEATURE PREPARATION + PROXY LABEL
# =========================================================
...
def apply_post_deployment_drift(df, period, sensitive_attr):
    ...
    if sensitive_attr == "age_group":
        ...
        mask = df["credit_score_group"] == "Low (300-550)"
        vulnerability = 0.50
...
class FairnessMonitor:
    ...
    def __init__(self, sensitive_attr):
        ...
        if sensitive_attr == "age_group":
            ...
            self.protected_label = "Low (300-550)"
            self.reference_label = "Medium (551-700)"
...
def main():
    ...
    print("Indian Loan Dataset - Data Drift Detection")
    print("=" * 80)
    ...
    print("      - credit_score_group: Low (300-550) vs Medium (551-700)")
...
if __name__ == "__main__":
    if CONFIG["USE_SEED"]:
        np.random.seed(CONFIG["SEED"])  # ✅ CORRECT INDENTATION
